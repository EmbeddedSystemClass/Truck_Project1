...
and fuel pump. (Now that you know how to start my truck, please don't take it. I guess I need a password dialog.)<br />
...
desktop. When it's in the vehicle, I have to use a Windows app called 'SendFiles' which makes a TCP/IP  connection<br />
...
There is also a 2 line LCD screen (not shown) on the iobox that scrolls the messages that come to the sched.<br />
<h2>The AVR/LCD screen</h2>
...
from UART2 of the STM32 and sent them to the LCD screen's parallel port. I was trying to do some kind of user<br />
input from the keypad which got too problematic so I went to using the Windows laptop as the UI. This is not<br />
show on the system diagram at the top, but I will have to, at some point re-install it when I start running out<br />
of room to display RT data on the Windows screen. All the code for the AVR processor is in the AVR_t6963<br />
directory. At one time I had a PWM signal coming from the FPGA board which would dim the LCD screen.<br />
...
cam and speakers for the laptop to play music or other mp3 files. I have a small battery charger hooked<br />
up to the 2nd car battery. The reason I had to use a 2nd car battery is the inverter craps out when running<br />
off the 12vdc input from the alternator - just too high a voltage. There is a 110v plug at the back of the<br />
truck which, in addition to the battery charger, runs a heat strip under the batteries, and runs a block<br />
heater on the engine block to keep it warm in the winter.<br />
<h2>Misc.</h2>
...
I should also mention that the buttons on the client screen: 'Reboot Server', 'Show Params', 'Home Server'<br />
and 'Set Srv Params' will be changed in the future. They are just for testing now.<br />
