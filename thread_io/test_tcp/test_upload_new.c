...
// test_upload_new.c - test the upload_new feature of laptop client (C#)
// client uploads new ARM exe (sched) to iobox using the UPLOAD_NEW cmd
//
#if 1
#include <stdio.h>
#include <stdlib.h>
...
//	setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, &option, sizeof(option));
	...
	do
	{
		...
		if (  (sd2=accept(sd, (struct sockaddr *)&cad, &alen)) < 0)
		{
			...
			printf("\n%s\n",strerror(errno));
			return -1;
		}else printf("connecting accepted\n");
	}while(sd2 < 1);
